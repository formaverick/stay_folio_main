				...
				<div class="reservation-status">
					...
					<div class="status-steps">
						<div
							...
							<p class="step-desc">호스트가 예약을 최종 확정한 상태입니다. 예약 취소 시 각 스테이의
								환불규정에 따라 환불이 진행됩니다.</p>
						</div>
						<div
							...
							<p class="step-desc">예약 내용에 따라 스테이를 이용할 수 있습니다. 예약 당일 오전 체크인
								안내를 드리고 있습니다.</p>
						</div>
						<div
				...
				<div class="reservation-detail-section">
					...
					<table class="reservation-info-table">
						<tr>
							...
							<td class="reservation-info-content">${reserv.srId } ( <c:choose>
									...
									<c:when test="${reserv.srStatus eq 'c'}">
										<fmt:formatDate value="${reserv.srCancledate }"
											pattern="yyyy.MM.dd HH:mm" />
									</c:when>
									<c:otherwise>
										<fmt:formatDate value="${reserv.srDate }"
											pattern="yyyy.MM.dd HH:mm" />
									</c:otherwise>
								</c:choose> )
							</td>
						...
						<tr>
							...
							<td class="reservation-info-content">${reserv.siName }/
								${reserv.riName }</td>
						</tr>
						<tr>
							<td class="reservation-info-title">03. 숙박 인원</td>
							<td class="reservation-info-content">총 ${reserv.srAdult + reserv.srChild }명
								(성인: ${reserv.srAdult }명 / 아동: ${reserv.srChild }명)</td>
						</tr>
						<tr>
							<td class="reservation-info-title">04. 체크인</td>
							<td class="reservation-info-content"><fmt:formatDate
									value="${reserv.srCheckin }" pattern="yyyy.MM.dd HH:mm" /></td>
						</tr>
						<tr>
							<td class="reservation-info-title">05. 체크아웃</td>
							<td class="reservation-info-content"><fmt:formatDate
									value="${reserv.srCheckout }" pattern="yyyy.MM.dd HH:mm" /></td>
						</tr>
						<tr>
							<td class="reservation-info-title">06. 요청사항</td>
							<td class="reservation-info-content">${reserv.srRequest ne null ? reserv.srRequest : '없음' }
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td class="reservation-info-content"><strong>숙박권의
									재판매, 양도, 양수, 교환을 금지합니다.</strong><br /> 예약자의 부득이한 사유로 인해 본인 이용이 어려울 경우,
								가족에 한해 해당 스테이에 가족관계임을 증명할 수 있는 서류(가족관계증명서 등)와 실제 이용하시는 분의 신분증을
								제시 후 이용이 가능합니다.</td>
						</tr>
					</table>
				</div>
						...
						<tr>
							...
							<td class="reservation-info-content">${reserv.srPayment } (
								<c:choose>
									...
									<c:otherwise>
										<fmt:formatDate value="${reserv.srPaydate }"
											pattern="yyyy.MM.dd HH:mm" />
									</c:otherwise>
								</c:choose> )
							</td>
